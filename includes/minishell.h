...
/*   Updated: 2025/02/13 15:45:15 by rreimann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define MARK_EXP_ERR \
	"marking for export is not possible since \
...
int								update_cmd_grp_fds(t_minishell *minishell,
									t_cmd_grp *cmd_grp_node, char *s1,
									char *s2);
...
accordingly.*/
int								update_cmd_grp_cmds(t_minishell *minishell,
									int *i, t_cmd_grp *cmd_grp_node);
...
void							ft_exit_parent(char **args,
									t_minishell *minishell);
/*Similar to exit builtin of bash, except it does not support flags/options.
Exits process in all cases (including error).*/
void							ft_exit_child(char **args,
									t_minishell *minishell);
...
void							execute_ith_cmd_grp(int i,
									t_minishell *minishell);
...
void							gc_add_to_allocs(void *ptr,
									t_minishell *minishell);
...
void							gc_add_to_open_fds(int fd,
									t_minishell *minishell);
...
void							gc_exit(t_minishell *minishell,
									int exit_status);
...
void							gc_free_2d_char_arr(char **arr,
									t_minishell *minishell);
...
If malloc fails, exit the program immediately.*/
char							**copy_2d_char_arr(char **arr,
									t_minishell *minishell);
...
void							vec_increase_size(t_minishell *minishell,
									t_vec *vec);
void							vec_push_copy(t_minishell *minishell,
									t_vec *vec, void *element);
void							vec_push_ref(t_minishell *minishell, t_vec *vec,
									void *element);
...
t_token							get_next_token(t_minishell *minishell,
									size_t index);
t_token							tokenize_space(t_minishell *minishell,
									size_t index);
t_token							tokenize_quote(t_minishell *minishell,
									size_t index, char quote);
t_token							tokenize_word(t_minishell *minishell,
									size_t index);
t_token							tokenize_operator(t_minishell *minishell,
									size_t index);
void							tokens_to_array(t_minishell *minishell,
									t_vec *vec);

char							*replace_environment_variables(\
...
char							*replace_env(t_minishell *minishell,
									char *word);
char							*get_env(t_minishell *minishell, char *key);
char							*cut_front(t_minishell *minishell, char *str,
									size_t cut_len);
...
char							*str_add_char(t_minishell *minishell, char *str,
									char c);
...
void							put_builtin_error(char *builtin_name,
									char *problem, char *msg);
...
char							*gc_ft_substr(char *s, unsigned int start,
									size_t len, t_minishell *minishell);
/*ft_strjoin's memory-safe alternative that utilizes the garbage collector.
Checks for `malloc` failure. Result can be freed with `gc_free`. Please do not
use `free` on the result.*/
char							*gc_ft_strjoin(char *s1, char *s2,
									t_minishell *minishell);
