...
/*   Updated: 2025/01/06 21:08:59 by anaqvi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	update_cmd_grp_cmds(t_minishell *minishell, int *i, t_cmd_grp *cmd_grp_node)
{
	...
	cmd_grp_node->cmd_name = minishell->tokenized[cmd_start];
	cmd_grp_node->cmd_args = copy_cmd_args(minishell, arg_count, cmd_start,
		cmd_grp_node);
	if (!(cmd_grp_node->cmd_args))
		return (-1);
	return (0);
}
...
{
	// replace cmd_grp_node->cmd_name with full path from env (for externals)
}
